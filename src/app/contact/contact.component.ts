...
export class ContactComponent implements OnInit {
  ...
  loadContacts() {
    ...
    if (userId) {
      // Récupérer et filtrer les contacts en fonction de l'utilisateur
      const storedContacts: Contact[] = JSON.parse(localStorage.getItem('contacts') || '[]');
      const storedTrash: Contact[] = JSON.parse(localStorage.getItem('trash') || '[]');

      this.contacts = storedContacts.filter((contact: Contact) =>
        contact.createdBy === userId
      );
      this.trash = storedTrash.filter((contact: Contact) =>
        contact.createdBy === userId
      );
    } else {
      this.contacts = [];
